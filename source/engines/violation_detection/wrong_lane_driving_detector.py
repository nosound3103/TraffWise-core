...
class WrongLaneDrivingDetector:
    ...
    def detect_violation(self, track_id, bbox, frame, speed):
        ...
        if np.linalg.norm(direction) > 1e-6:
            ...
            dot_product = np.dot(
                direction, self.lanes[lane_index].expected_direction)
            dot_product = np.clip(dot_product, -1.0, 1.0)
            angle_diff = np.degrees(np.arccos(dot_product))
            turn_type = self.classify_turn(
                self.lanes[lane_index].expected_direction, direction)
            ...
            if dot_product < self.dot_threshold or angle_diff > self.angle_threshold:
                if speed > 3:
                    self.violations_count[track_id] = self.violations_count.get(
                        track_id, 0) + 1
    ...
    def capture_violation(self, frame, log):
        ...
        track_log = log["track_id"]
        public_id_prefix = f"{folder_path}/{track_log}"
